...
int main(){
    ...
    printf("PAREMATERS:\n    Hidden Layer Size = %d \n    Training Data Size = %d \n    eta = %f \n    Number of Epochs = %d\n    In Constant = %f/n", LAYER2SIZE, TRAININGDATASIZE, eta, NUMTRAININGEPOCHS, inConstant);
...
void normalize8BitValues(float* data, int size){
    int i;
    for (i = 0; i < size; i++){
        data[i] = sigmoid(inConstant * ((*(data + i) / 255.0) - .5));
    }
}
